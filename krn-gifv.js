var _KRNGIFv = {
    start: function(cb) {
        var self = this;

        if(cb) {
            self.cb = cb;
        }
        self.video_elements = $('[data-krn-gifv="1"]');
        self.isAutoPlayAthing();
    },
    isAutoPlayAthing: function() {
        var self = this;

        var src = 'data:video/mp4;base64,';
        var video_check = $('<video src="'+src+'" controls playsinline muted width="20" height="20" style="visibility:hidden; position:fixed;z-index:1000;top:0;left:0"></video>');

        var brain = Cookies.get('krn_gifv_check');
        if(brain) {
            if(brain == 'no') {
                self.fallBackPosters();
                return;
            }
            if(brain == 'yes') {
                self.autoPlayWorks();
                return;
            }
        }
        // firefox on android issue -> serve poster image
        var isAndroid = navigator.userAgent.search('Android');
        var isFirefox = navigator.userAgent.search('Firefox');
        if(isAndroid > -1 && isFirefox > -1) {
            self.fallBackPosters();
            return;
        }

        video_check[0].onplay = function() {
            self.playing = true;
        };
        video_check[0].oncanplay = function() {
            if(!self.hasPromise && self.playing) {
                self.autoPlayWorks();
            }
            if(!self.hasPromise && !self.playing) {
                self.fallBackPosters();
            }
        };

        self.hasPromise = false;
        $('body').prepend(video_check);
        play_promise = video_check[0].play();

        //Modern Browser should support Promise based interface, otherwise just
        //serve posters
        if(play_promise !== undefined) {
            self.hasPromise = true;
            play_promise.then(function() {
                self.autoPlayWorks();
            })
            .catch(function(e) {
                self.fallBackPosters();
            });
        } else {
            self.hasPromise = false;
            //console.log('WAITING FOR FALLBACK METHOD');
        }
    },
    fallBackPosters: function() {
        var self = this;

        Cookies.set('krn_gifv_check', 'no');
        self.video_elements.each(function() {
            $(this).attr('poster', $(this).data('krn-poster'));
        })
    },
    autoPlayWorks: function() {
        var self = this;

        Cookies.set('krn_gifv_check', 'yes');
        self.video_elements.each(function(idx) {
            if(self.cb) {
                if(self.cb('play', idx)) {
                    self.initEvent(this);
                } else {
                    $(this).attr('poster', $(this).data('krn-poster'));
                }
            } else {
                self.initEvent(this);
            }
        })
    },
    initEvent: function(elem) {
        var self = this;

        if('IntersectionObserver' in window) {
            // console.log('krn-gifv:', 'intersectionobserver variante');
            var videoObserver = new IntersectionObserver(function(entries, observer) {
                entries.forEach(function(entry) {
                    var $video = $(entry.target);
                    var vPoster = $video.data('poster');
                    var vKRNPoster = $video.data('krn-poster');
                    var vSrc = $video.data('src');
                    var vAlt = $video.data('alt');
                    var vTitle = $video.data('title');
                    var vClass = $video.data('class');
                    var vHeight = $video.data('height');
                    var vWidth = $video.data('width');

                    var vTemplate =
                        '<video' +
                            (vPoster ? ' poster="' + vPoster + '"' : '') +
                            (vKRNPoster ? ' data-krn-poster="' + vKRNPoster + '"' : '') +
                            (vWidth ? ' width="' + vWidth + '"' : '') +
                            (vHeight ? ' height="' + vHeight + '"' : '') +
                            (vAlt ? ' alt="' + vAlt + '"' : '') +
                            (vTitle ? ' title="' + vTitle + '"' : '') +
                            (vClass ? ' class="' + vClass + '"' : '') +
                            ' playsinline loop muted' +
                        '>' +
                            '<source' +
                                ' type="video/mp4"' +
                                (vSrc ? ' src="' + vSrc + '"' : '') +
                            '/>' +
                        '</video>'
                    ;
                    if(entry.isIntersecting) {
                        // console.log('krn-gifv:', 'play');
                        if(!$video.find('video').length) {
                            $video.html(vTemplate);
                            $video.find('video')[0].play();
                        }
                    } else {
                        // console.log('krn-gifv:', 'pause');
                        $video.empty();
                    }
                });
            });

            var videos = [].slice.call(self.video_elements);
            videos.forEach(function(video) {
                videoObserver.observe(video);
            });
        } else {
            // console.log('krn-gifv:', 'events variante');
            function videoPlayPause() {
                var $video = $(elem);
                var vPoster = $video.data('poster');
                var vKRNPoster = $video.data('krn-poster');
                var vSrc = $video.data('src');
                var vAlt = $video.data('alt');
                var vTitle = $video.data('title');
                var vClass = $video.data('class');
                var vHeight = $video.data('height');
                var vWidth = $video.data('width');

                var vTemplate =
                    '<video' +
                        (vPoster ? ' poster="' + vPoster + '"' : '') +
                        (vKRNPoster ? ' data-krn-poster="' + vKRNPoster + '"' : '') +
                        (vWidth ? ' width="' + vWidth + '"' : '') +
                        (vHeight ? ' height="' + vHeight + '"' : '') +
                        (vAlt ? ' alt="' + vAlt + '"' : '') +
                        (vTitle ? ' title="' + vTitle + '"' : '') +
                        (vClass ? ' class="' + vClass + '"' : '') +
                        ' playsinline loop muted' +
                    '>' +
                        '<source' +
                            ' type="video/mp4"' +
                            (vSrc ? ' src="' + vSrc + '"' : '') +
                        '/>' +
                    '</video>'
                ;
                if(self.isInViewport($video)) {
                    // console.log('krn-gifv:', 'play');
                    if(!$video.find('video').length) {
                        $video.html(vTemplate);
                        $video.find('video')[0].play();
                    }
                } else {
                    // console.log('krn-gifv:', 'pause');
                    $video.empty();
                }
            }
            window.addEventListener('scroll', videoPlayPause, { passive: true });
            window.addEventListener('resize', videoPlayPause, { passive: true });
            window.addEventListener('orientationchange', videoPlayPause, { passive: true });
        }
    },
    isInViewport: function(elem) {
        var self = this;

        if(!$(elem).length) {
            return false;
        }
        var elementTop = $(elem).offset().top;
        var elementBottom = elementTop + $(elem).outerHeight();
        var viewportTop = $(window).scrollTop();
        var viewportBottom = viewportTop + $(window).height();
        return elementBottom > viewportTop && elementTop < viewportBottom;
    }

}

if(typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
    module.exports = _KRNGIFv;
} else {
    window.KRNGIFv = _KRNGIFv;
}
